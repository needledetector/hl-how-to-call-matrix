...
try {
  ...
  assert.equal(await evaluate(`document.querySelector('#bShort input[value="full"]').checked`), true);
  ...
  await evaluate(`window.originalCell = document.querySelector('#mx td')`);
  await click('#themeToggle');
  assert.equal(await evaluate(`document.documentElement.dataset.theme`), 'dark');
  assert.equal(await evaluate(`document.querySelector('#mx td') === window.originalCell`), true);
  assert.equal(await evaluate(`getComputedStyle(document.body).backgroundColor`), 'rgb(17, 22, 30)');
  await screenshot('desktop-dark');
  await change('#bShort', 'short');
  assert.equal(await evaluate(`document.querySelector('#mx td') === window.originalCell`), true);
  assert.equal(await evaluate(`document.querySelector('#mx th[data-person="p0"]').textContent`), 'あおい');
  await change('#bShort', 'full');
  assert.equal(await evaluate(`document.querySelector('#mx th[data-person="p0"]').textContent`), '青空あおい');
  ...
  await evaluate(`document.documentElement.dataset.beforeReload = '1'`);
  await send('Page.reload');
  await waitFor(`!document.documentElement.dataset.beforeReload && document.querySelector('#resultSummary')?.textContent`);
  assert.equal(await evaluate(`document.documentElement.dataset.theme`), 'dark');
  ...
  assert.equal(await evaluate(`getComputedStyle(document.querySelector('.card-detail')).color`), 'rgb(193, 197, 202)');
